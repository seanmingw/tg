...
init_var() {
  ...
  # Cert
  CERT_PATH="${TP_DATA}cert/"
  DOMAIN_FILE="${TP_DATA}domain.lock"
  domain=""
  crt_path=""
  key_path=""

  # Caddy2
  CADDY_DATA="${TP_DATA}caddy/"
  ...
  # Redis
  REDIS_DATA="${TP_DATA}redis/"
  redis_host="127.0.0.1"
  redis_port=6378
  redis_pass=""

  # Trojan Panel Frontend
  TROJAN_PANEL_UI_DATA="${TP_DATA}trojan-panel-ui/"
  # Nginx
  ...
  TROJAN_PANEL_DATA="${TP_DATA}trojan-panel/"
  ...
  # Trojan Panel Core
  TROJAN_PANEL_CORE_DATA="${TP_DATA}trojan-panel-core/"
...
check_sys() {
  ...
  if [[ "${release}" != "macos" ]]; then
    ...
    if [[ -z "${release}" ]]; then
      echo_content red "The operating system only supports CentOS 7+/Ubuntu 18+/Debian 10+/macOS"
      exit 0
    fi
  fi
...
install_docker() {
  if [[ ! $(docker -v 2>/dev/null) ]]; then
    ...
    if [[ "${release}" == "macos" ]]; then
      ...
      echo_content yellow "---> Please start Docker Desktop manually from Applications folder"
      echo_content yellow "---> Press any key after Docker Desktop is running..."
      read -n 1
    else
      # turn off firewall
