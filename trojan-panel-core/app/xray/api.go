...
func (x *xrayApi) HandleConnection(hash, clientIP string, conn net.Conn, protocol string) error {
	// 生成连接ID
	connectionId := util.GenerateUUID()
	
	// 添加连接到流量控制器
	err := x.trafficController.AddConnection(hash, connectionId, clientIP, fmt.Sprintf("xray-%s", protocol), conn)
	if err != nil {
		logrus.Errorf("Xray-%s添加连接失败: %v", protocol, err)
		return err
	}
	...
	go func() {
		defer func() {
			// 连接关闭时统计流量并移除连接
			download, upload := x.getConnectionTraffic(conn)
			x.trafficController.RemoveConnection(hash, connectionId, download, upload)
		}()
...
func (x *xrayApi) getConnectionTraffic(conn net.Conn) (download, upload uint64) {
